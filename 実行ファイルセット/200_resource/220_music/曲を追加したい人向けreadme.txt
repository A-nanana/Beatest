...
3.ファイルネストに従ってファイルを追加する(propaty.txtは必須ではないが、入れるとデータベースが開かなかったときもできるようになる)
	propaty.txtの中身(先頭の文字は開けない,改行で区切る)
		Bpm
		時間
		譜面の1小節に係る拍子

4.Lineup.txtにタイトルを入れる(必須ではないが推奨)
        好きなところに改行してタイトルを入れると、データベースが開かなかったときにとりあえずプレイできます。
        データベースを消さない限り、未記入で問題ありません。
