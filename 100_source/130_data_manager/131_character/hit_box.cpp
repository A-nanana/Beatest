...
void HitBox::SetBox(float x_start, float y_start, float x_size, float y_size, float lotate_rad)
{
	//回転の設定
	lotate_rad_ = lotate_rad;
	Matrix2 matrix;
...
bool HitBox::HitCheckToPoint(Vector2D* other)
{
	for (int i = 0; i < hit_set::squair_point; i++) {
		//頂点へのベクトル
		Vector3D to_point_(other->x_ - point_[i].x_, other->y_ - point_[i].y_, NULL);
		//計算結果用
		Vector3D result(vectol_[i]);
		result.Cross(to_point_);
		//外積が0以上なら当たり判定の外
		if ( result.z_ > 0) {
			return false;
		}
